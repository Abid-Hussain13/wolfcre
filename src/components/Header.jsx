      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            ...
            <nav className="flex flex-col items-center gap-y-6 md:gap-y-10">
              {navLinks.map((link, index) => (
                <div key={link.name} className="overflow-hidden">
                  ...
                  >
                    <Link
                      to={link.href}
                      className="text-4xl md:text-8xl font-serif text-white hover:text-luxury-gold italic tracking-tighter transition-all duration-500 flex items-end gap-6"
                    >
                      <span className="text-xs not-italic font-sans text-white/20 mb-2 md:mb-6">{`0${index + 1}`}</span>
                      {link.name}
                    </Link>
                  </motion.div>
                </div>
            ...
            <motion.div 
              ...
              className="absolute bottom-20 flex gap-12 text-[9px] uppercase tracking-[0.4em] text-white/40"
            >
